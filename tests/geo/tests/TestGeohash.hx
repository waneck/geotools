...
class TestGeohash
{
  ...
  public function test_geohash()
  {
    ...
    var north = new Geohash('dqcjq').neighbor(new Location(1,0));
    Assert.equals(north, 'dqcjw');

    var southwest = new Geohash('DQCJQ').neighbor(new Location(-1,-1));
    Assert.equals(southwest, 'dqcjj');

    var bboxes = Geohash.fromRange( new Range( 30, 30.0001, 120, 120.0001), 8);
    Assert.equals(bboxes[bboxes.length-1], geohash(new Location(30.0001,120.0001), 8));
  }
